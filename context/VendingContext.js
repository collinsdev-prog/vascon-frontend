...
export const VendingProvider = ({ children }) => {
  ...
  const depositCoins = useCallback(
    async (amount) => {
      try {
        ...
        toast.success(`Successfully deposited ${formatCentsToDollars(amount)}`);
  ...
  const handleBuy = useCallback(
    async (productId, quantity, productPrice) => {
      ...
      if (deposit < totalCost) {
        setChangeMessage(
          "Insufficient funds. Please deposit to make a purchase."
        );
        return;
      }
      try {
        ...
        if (res?.data) {
          const { change, note, products, depositedAmount, totalSpent } =
            res.data;
          ...
          setChangeMessage(
            change?.length
              ? `✅ Purchase successful. Your change: ${change
                  .map((c) => formatCentsToDollars(c))
                  .join(", ")}`
        ...
        toast.error(errorMessage); // Notify user with toast
        setChangeMessage(`❌ ${errorMessage}`); //also show in the Alert card
